	...
	if os.path.exists(f):
		for fileName in os.listdir(f):
			try:
				...
				print("Please close file "+fileName+" and try again\n")
				quit(0)
...
def removeemptytags(elem):
    ...
    for child in elem:
	    ...
	    #Added for 'unit' tag
            if 'unit' in child.attrib and (child.attrib['unit'] == None or child.attrib['unit'] == ""):
                del child.attrib['unit']
        #if True:
        #        print (len(child))
        if child is None or (len(child) == 0 and child.text is not None and len(child.text) == 0 and len(child.attrib) == 0):
