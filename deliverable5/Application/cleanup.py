...
def checkForBinaryPlanet(root, criteria, liststring, filename):
...
def checkForTransitingPlanets(root, filename):
...
def cleanUp():
    ...
    global xmlerrors
    global issues    
    # Loop over all files in changed_systems and  create new data
    for filename in glob.glob("Changed_Systems/*.xml"):
        fileschecked += 1
        ...
        checkForBinaryPlanet(root, ".//binary/planet", "Planets in binary systems, P-type", filename)
        checkForBinaryPlanet(root, ".//binary/star/planet", "Planets in binary systems, S-type", filename)
        ...
        checkForTransitingPlanets(root, filename)
